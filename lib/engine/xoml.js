...
Xoml.loadJSON = function(filename, blocking, callback)
{
...
Xoml.prototype.parseBrush = function(parent, item)
{
    ...
    if (item.BrushType == 0)
    {
        ...
        if (frames.length == 0)
            brush = new ImageAtlas(item.Name, bitmap, rect.x, rect.y, rect.w, rect.h);
        else
        {
            // Parse frames
            brush = new ImageAtlas(item.Name, bitmap);
...
Xoml.prototype.parseActor = function(actor, parent, item)
{
    ...
    if (item.GAngle !== undefined) actor.grad_rotation = item.GAngle;
    ...
    if (item.Docking !== undefined)
    {
        ...
        if (docking == "top" || docking == "topleft" || docking == "topright")
            ...
            actor.dock_y = Actor.Dock_Bottom;
    ...
    if (item.Background !== undefined)
    {
        ...
        if (brush.stops !== undefined)
        {
            ...
            var x2 = w;
            var y2 = 0;
            var angle = (actor.grad_rotation * Math.PI) / 180;
            var xt1 = x1 * Math.cos(angle) + y1 * Math.sin(angle);
            var yt1 = x1 * -Math.sin(angle) + y1 * Math.cos(angle);
            var xt2 = x2 * Math.cos(angle) + y2 * Math.sin(angle);
            var yt2 = x2 * -Math.sin(angle) + y2 * Math.cos(angle);
            var grad = this.app.display.context.createLinearGradient(xt1, yt1, xt2, yt2);
            for (var t = 0; t < brush.stops.length; t++)
            {
                var s = brush.stops[t];
...
Xoml.prototype.parseIcon = function(parent, item)
{
    ...
    if (render_as == 0 && item.Background !== undefined)
    {
        // If gradient brush attached to an image actor then convert it to a rect actor
        var brush = this.current_scene.findResource(item.Background, "brush");
        if (brush.stops !== undefined)
            render_as = 2;
    }
...
Xoml.prototype.parseLabel = function(parent, item)
{
    ...
    this.parseActor(actor, parent, item);
    if (item.TextColour !== undefined)
        actor.fill_style = item.TextColour;
