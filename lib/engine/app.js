...
//    this.timer = null;				// Main loop timer
    ...
    this.loading_screen = {
        background_fill: "#fffff",              // Loading background fill
        background_image: "loading.png",        // Loading background
...
TheApp.prototype.countResourcesNeedLoading = function(include_scenes)
{
    var total = 0;
...
TheApp.prototype.onResourceLoadedBase = function(resource, error)
{
    if (resource.preload)
        window.app.total_loaded++;
    if (error)
    {
        if (resource.preload)
            window.app.total_load_errors++;
...
TheApp.prototype.waitForResources = function()
{
    // Draw background
    var app = this;
    ...
    var load_w = 400;
    var load_h = 80;
    var context = disp.context;
    context.setTransform(1, 0, 0, 1, 0, 0);
    context.fillStyle = app.loading_screen.background_fill;
    context.fillRect(0, 0, app.display_width, app.display_height);
    var bg = new Image();
    bg.src = app.loading_screen.background_image;
    bg.onload = function() {
        context.setTransform(dscale, 0, 0, dscale, app.canvas_cx, app.canvas_cy);
        context.drawImage(bg, -bg.width >> 1, -bg.height >> 1, bg.width, bg.height);
        context.fillStyle = app.loading_screen.bar_background_fill;
    ...
    {
        var loaded = window.app.total_loaded;
        if (window.app.total_loaded >= total)
        {
            if (window.app.debug)
            {
                console.log("Total resources loaded " + loaded + " of " + total);
                console.log("Total resource load errors " + window.app.total_load_errors);
            }
        ...
        var cw = (w * perc) << 0;
        context.fillStyle = app.loading_screen.bar_fill;
...
{
    new Xoml(this).parseResources(this, [window[scene_name]]);
