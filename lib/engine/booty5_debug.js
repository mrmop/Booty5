 ...
 * @property {number}                   ox                  - origin (between -1 and 1), set via _ox, if value falls outside that range then origin will be interpreted as pixels (default is 0)
 ...
 * - points {number[]} – An array of points of form [x1, y1, x2, y2 etc] that define a shape (if type is polygon)
...
b5.App = function(canvas, web_audio)
{
    ...
    this.use_web_audio = web_audio;             // If true then Web Audio will be used if its available (default is true)
 ...
 *               "GeometryType": "Poly",
 *               "Absolute": false,
 *               "Vertices": [-126.336,-106.399, -42.018,-90.992, 153.614,46.411, -66.468,106.4, -153.614,-7.28]
 *           },
 *           {
 *               "ResourceType": "Shape",
 *               "Name": "rect3761",
...
b5.Xoml.prototype.parseSound = function(parent, item)
{
    ...
    if (item.AutoPlay !== undefined)
        sound.auto_play = item.AutoPlay;
...
b5.Xoml.prototype.parseActor = function(actor, parent, item)
{
    ...
    if (item.RenderAs === 3)
    {
        var tiles_x = item.TilesX;
        var tiles_y = item.TilesY;
        actor.map_width = item.MapW;
        actor.map_height = item.MapH;
        actor.tile_width = item.TileW;
        actor.tile_height = item.TileH;
        actor.display_width = item.DispW;
        actor.display_height = item.DispH;
        actor.map = item.Map;
    ...
    if (fixtures !== undefined)
    {
        for (var t = 0; t < fixtures.length; t++)
        {
            ...
            if (fixtures[t].CollisionFlags !== undefined)
            {
                options.collision_category = fixtures[t].CollisionFlags.x;
                options.collision_mask = fixtures[t].CollisionFlags.y;
                options.collision_group = fixtures[t].CollisionFlags.z;
            }
 ...
 * @property {number}                   restitution     - Material restitution, lower values make he object more bouncy
