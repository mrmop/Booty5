			...
			tile.guess_actor.onTapped = function() {
				if (window.shuffle_match.allow_shuffle)
				{
					...
					if (sm_tile.type >= 0)
					{
						...
						sm_tile.guess_actor.scene.timelines.add(new b5.Timeline(sm_tile.guess_actor, "_scale", [1, 1.2, 1], [0, 0.25, 0.5], 1, [b5.Ease.backin, b5.Ease.backout]));
					}
				}
			};
...
Board.prototype.flashTargetNumbers = function()
{
	var target = window.shuffle_match.target_number;

	var tiles = this.tiles;
	var count = tiles.length;
	for (var t = 0; t < count; t++)
	{
		if (tiles[t].type === target)
		{
			tiles[t].guess_actor.atlas = b5.app.findResource("d" + target, "brush");
		}
	}
	...
	b5.Utils.RunafterTime(1, function()
	{
		for (var t = 0; t < count; t++)
		{
			if (tiles[t].type === target)
			{
				tiles[t].guess_actor.atlas = b5.app.findResource("empty", "brush");
			}
		}				
	})	
};
